...
// ----------------------------------------------------------
// constructor for struct Coordinate
// ----------------------------------------------------------
...
// ----------------------------------------------------------
// triangle face constructor for struct Face
// ----------------------------------------------------------
...
// ----------------------------------------------------------
// quad face constructor for struct Face
// ----------------------------------------------------------
...
// ----------------------------------------------------------
// constructor for struct Material
// ----------------------------------------------------------
...
// ----------------------------------------------------------
// constructor for struct TextCoord
// ----------------------------------------------------------
...
// ----------------------------------------------------------
// the main model load function taking in obj filename and return list identifier
// ----------------------------------------------------------
int ObjLoader::load(const char* filename)
{
	...
	while (!in.eof())
	{
		in.getline(buf, 256);	//while not in the end of the file, read everything as a string to the coord vector
		coord.push_back(new std::string(buf));
	}
	for (std::size_t i = 0; i < coord.size(); i++)	//and then go through all line and decide what kind of line it is
	{
		...
		else if ((*coord[i])[0] == 'f')	//if it's a face
		{
			...
			if (count(coord[i]->begin(), coord[i]->end(), ' ') == 4)	//if this is a quad
			{
				if (coord[i]->find("//") != std::string::npos)	//if it's contain a normal vector, but not contain texture coorinate
				{
					...
					faces.push_back(new Face(b, a, c, d, e, 0, 0, 0, 0, curmat));	//and put to the faces, don't care about the texture coorinate in this case
				...
				else if (coord[i]->find("/") != std::string::npos)	//if we have texture coorinate and normal vectors
				{
					...
					//else no normal vectors nor texture coorinate
					sscanf(coord[i]->c_str(), "f %d %d %d %d", &a, &b, &c, &d);
					faces.push_back(new Face(-1, a, b, c, d, 0, 0, 0, 0, curmat));
				}
			}
			else {	//if it's a triangle
					//do the same, except using one less vertex/texture coorinate/face number
		...
		else if ((*coord[i])[0] == 'u' && (*coord[i])[1] == 's' && (*coord[i])[2] == 'e')	//use material material_name
		{
			...
			for (std::size_t i = 0; i < materials.size(); i++)	//go through all of the materials
			{
		...
		{
			char filen[200];
			char filena[200] = "character_walkingupstairs/";
			sscanf(coord[i]->c_str(), "mtllib %s", filen);	//read the filename
			std::strcat(filena, filen);
			std::ifstream mtlin(filena);	//open the file
			...
			bool ismat = false;	//already have a material read in to these variables?
			strcpy(filename, "\0");	//set filename to nullbyte character
			for (std::size_t i = 0; i < tmp.size(); i++) //go through all lines of the mtllib file
			{
				...
				if (tmp[i][0] == 'n' && tmp[i][1] == 'e' && tmp[i][2] == 'w')	//new material
				{
					...
					ismat = false;	//start from a fresh material
				...
				else if (tmp[i][0] == 'i' && tmp[i][1] == 'l')	//the illum 
				{
				...
				{
					char actualFilename[200] = "character_walkingupstairs/";
					sscanf(tmp[i].c_str(), "map_Kd %s", filename);
					std::strcat(actualFilename, filename);
					texture = loadTexture(actualFilename);	//read the filename, and use the loadTexture function to load it, and get the id.
					ismat = true;
				}
			}
	...
	glNewList(num, GL_COMPILE); //I create a new display list
	int last = -1;	//the last material (default -1, which doesn't exist, so I use the first material)
	for (std::size_t i = 0; i < faces.size(); i++)	//go throught all faces
	{
		if (last != faces[i]->mat && ismaterial)	//if I have a meterial AND the last material is not the same
		{
			...
			if (materials[faces[i]->mat]->texture == -1)	//if I don't have texture, disable it, else enable it
		...
		if (faces[i]->four)	//if quad, check for normals and textures and draw
		{
			glBegin(GL_QUADS);
			if (isnormals)	
		...
		else {	//if triangle, check for normals and textures and draw
	...
	glEndList(); //I replace the display list
	clean();	//free memory
	lists.push_back(num);	//append to list
	return num;
}
...
void ObjLoader::clean()
{
	...
	for (std::size_t i = 0; i < materials.size(); i++)
		delete materials[i];
	for (std::size_t i = 0; i < texturecoordinate.size(); i++)
...
// ----------------------------------------------------------
// load the filename textures (PNG format) and return texture name
// ----------------------------------------------------------
unsigned int ObjLoader::loadTexture(const char* filename)
{
	unsigned int num;		//location where generated texture names are stored
	int width, height;		//texture image width, height	
	unsigned char* img;		//use for loading image

	//generate 1 texture name, use SOIL to load PNG as texture, then bind
	glGenTextures(1, &num);
	...
	//can't open png file
	if (img == NULL) {
		std::cout << "cannot open the png file" << std::endl;
	}

	//have OpenGL use blurry texture, specify 2d texture, set texture environment 
	...
	//append texture name to texture
	texture.push_back(num);
	return num;
}

// ----------------------------------------------------------
// constructor for class ObjLoader
// ----------------------------------------------------------
ObjLoader::ObjLoader()
{
	//at default I set all booleans to false
	ismaterial = false;
	isnormals = false;
	istexture = false;
}